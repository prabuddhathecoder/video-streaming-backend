...
const getAllVideos = asyncHandler(async (req, res) => {
    ...
    try {
        // Get ALL videos from Video collection
        const videos = await Video.find().skip((page - 1) * limit).limit(parseInt(limit)).populate('owner', 'username avatar');

        // Check if any videos exist
        if (!videos || videos.length === 0) {
            throw new ApiError(404, "No videos found in database");
        }
